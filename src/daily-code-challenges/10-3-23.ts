...
const binaryFingers = (binString: string): string[] => {
  // const fingers = [];
  // if (binString == '') {
  //   return [];
  // }

  // let num = Number(binString);
  // if (num >= 10000) {
  //   fingers.push('Pinkie');
  //   num = num % 10000;
  // }
  ...
  const countedFingers = [];

  binString = binString.padStart(5, '0');

  for (let i = 0; i < binString.length; i++) {
    if (binString[i] == '1') countedFingers.push(fingers[i]);
  }

  return countedFingers;
};

export default binaryFingers;
